...
build. There is a shell script to [run](root/opt/run.sh) it which runs when the
container starts.
...
to open a secure WebSocket on port 9001 and sends a single message. If
successful, it prints the response sent from the server.
